const WORLD_CUP_NEXT_TARGET_BY_ROUND_SLOT = {
  ...
  "F4|1": { round: "CHIP", slot: 1, side: "team1_id" },
  "F4|2": { round: "CHIP", slot: 1, side: "team2_id" },
};

const WORLD_CUP_KNOCKOUT_ROUNDS = new Set(["R32", "S16", "E8", "F4", "CHIP"]);

const WORLD_CUP_PENALTY_ADVANCERS_BY_ROUND_SLOT = new Map([
  ["R32|6", "team2_id"],
  ["R32|13", "team2_id"],
  ["R32|16", "team2_id"],
  ["S16|4", "team1_id"],
...
export function getWorldCupManualWinnerId(game) {
  return getWorldCupOfficialWinnerId(game);
}

export function getWorldCupOfficialWinnerId(game) {
  const round = normalizedRound(game.round);
  if (!WORLD_CUP_KNOCKOUT_ROUNDS.has(round)) {
    return game.winner_team_id ?? null;
  }

  const team1Id = game.team1_id ?? null;
  const team2Id = game.team2_id ?? null;
  if (!team1Id || !team2Id) return null;

  if (isFinalStatus(game.status) && typeof game.team1_score === "number" && typeof game.team2_score === "number") {
    if (game.team1_score > game.team2_score) return team1Id;
    if (game.team2_score > game.team1_score) return team2Id;

    const persistedWinner = game.winner_team_id ?? null;
    if (persistedWinner === team1Id || persistedWinner === team2Id) return persistedWinner;

    const slot = numericSlot(game.slot);
    const penaltyAdvancerSide =
      slot == null ? null : WORLD_CUP_PENALTY_ADVANCERS_BY_ROUND_SLOT.get(`${round}|${slot}`);
    if (penaltyAdvancerSide === "team1_id") return team1Id;
    if (penaltyAdvancerSide === "team2_id") return team2Id;
    return null;
  }

  return game.winner_team_id === team1Id || game.winner_team_id === team2Id
    ? game.winner_team_id
    : null;
}
